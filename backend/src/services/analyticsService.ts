...
export async function getDashBoardsStatsService(startDate: string, endDate: string): Promise<AnalyticsServiceResult> {
    try {
        ...
        // 1. Total de Receita e Pedidos (apenas status confirmados)
        ...
        // 2. Pedidos por Status (todos os status)
        ...
        // 3. Pedidos por Dia (últimos 7 dias) - CORRIGIDO
        ...
        const ordersByDay = await Order.aggregate([
            {
                $match: {
                    ...
                    status: { $in: ['paid', 'shipped', 'delivered'] } // Apenas pedidos confirmados
                }
            },
            {
                $group: {
                    _id: { 
                        $dateToString: { 
                            format: '%Y-%m-%d', 
                            date: '$createdAt',
                            timezone: 'America/Sao_Paulo' // Ajusta para fuso BR
                        } 
                    },
                    orders: { $sum: 1 },
        ...
        // 4. Pedidos por Mês (últimos 6 meses) - CORRIGIDO
        ...
        // 5. Top Produtos Mais Vendidos - CORRIGIDO
        const topProducts = await Order.aggregate([
            { 
                $match: { 
                    status: { $in: ['paid', 'shipped', 'delivered'] },
                    createdAt: { $gte: start, $lte: end } // Filtra por período
                } 
            },
            { $unwind: '$items' },
            {
